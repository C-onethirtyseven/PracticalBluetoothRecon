...
class ScanService : Service() {
    ...
    private var lastCellScanMs = 0L
    private var lastNotifyMs = 0L
    private var lastLocationMs = 0L
    private var lastLat: Double? = null
    private var lastLon: Double? = null
    private var dwellMode = false
    private var scanMode = SCAN_MODE_BALANCED
    private var scanIntervalMs = 15000L
    private var wifiMinIntervalMs = 30000L
    private var cellIntervalMs = 15000L
    ...
    private val scanLoop = object : Runnable {
        override fun run() {
            ...
            readWifiSnapshot()
            if (System.currentTimeMillis() - lastCellScanMs >= cellIntervalMs) {
                lastCellScanMs = System.currentTimeMillis()
                scanCellInfo()
            }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                ...
                scanMode = intent.getStringExtra(EXTRA_SCAN_MODE) ?: scanMode
                applyScanMode()
                startScanning()
            }
            ...
            ACTION_SET_MODE -> {
                scanMode = intent.getStringExtra(EXTRA_SCAN_MODE) ?: SCAN_MODE_BALANCED
                applyScanMode()
            }
        }
        return START_STICKY
    ...
    private fun startScanning() {
        ...
        handler.postDelayed(scanLoop, scanIntervalMs)
        notifyStatus()
    }
    ...
    private fun startWifiScan() {
        ...
        if (now - lastWifiScanMs < wifiMinIntervalMs) {
            return
        }
        lastWifiScanMs = now
        try {
    ...
    private fun startBleScan() {
        val settings = ScanSettings.Builder()
            .setScanMode(if (scanMode == SCAN_MODE_LOW) ScanSettings.SCAN_MODE_LOW_POWER else ScanSettings.SCAN_MODE_LOW_LATENCY)
            .build()
    ...
    private fun applyScanMode() {
        when (scanMode) {
            SCAN_MODE_HIGH -> {
                scanIntervalMs = 15000L
                wifiMinIntervalMs = 30000L
                cellIntervalMs = 15000L
            }
            SCAN_MODE_LOW -> {
                scanIntervalMs = 60000L
                wifiMinIntervalMs = 120000L
                cellIntervalMs = 60000L
            }
            else -> {
                scanIntervalMs = 30000L
                wifiMinIntervalMs = 30000L
                cellIntervalMs = 30000L
            }
        }
    }
    ...
    companion object {
        ...
        const val ACTION_SET_MODE = "com.example.wifibtlogger.MODE"
