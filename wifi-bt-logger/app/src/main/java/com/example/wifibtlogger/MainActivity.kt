...
class MainActivity : AppCompatActivity() {
    ...
    private fun removeWhitelistedFromRunFiles(key: String) {
        ...
        val csvUri = if (csvPath != null) {
            if (csvPath.startsWith("content://")) Uri.parse(csvPath) else Uri.fromFile(File(csvPath))
        } else null
        val kmlUri = if (kmlPath != null) {
            if (kmlPath.startsWith("content://")) Uri.parse(kmlPath) else Uri.fromFile(File(kmlPath))
        } else null
    ...
    private fun ensurePinSetup() {
        val prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        if (prefs.getString(PREF_PIN, null) != null) return
    ...
    private fun promptPinAndRun(action: () -> Unit) {
        val prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val saved = prefs.getString(PREF_PIN, null) ?: return
        val input = EditText(this).apply {
            inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_VARIATION_PASSWORD
        }
        AlertDialog.Builder(this)
    ...
    private fun saveDwellFiles(stamp: String) {
        ...
        val uri = if (dwellCsv.startsWith("content://")) Uri.parse(dwellCsv) else Uri.fromFile(File(dwellCsv))
