...
def main() -> int:
    ...
    # Also create a repo zip from RepoApproved with the same version
    repo_dir = Path("/Users/kyledavis/Documents/RepoApproved/wifi-bt-logger")
    repo_zip = Path("/Users/kyledavis/Documents/RepoApproved") / f"PBR_repo_v{version}.zip"
    if repo_dir.exists():
        import zipfile
        with zipfile.ZipFile(repo_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for p in repo_dir.rglob("*"):
                if p.is_dir():
                    continue
                zf.write(p, arcname=str(p.relative_to(repo_dir)))
        print(f"Wrote {repo_zip}")
    return 0
